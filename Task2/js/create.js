...
/*
Создать наследника класса Menu – новый класс должен уметь строить меню со вложенными пунктами, 
т.е с подменю. Подсказка: главный секрет в обходе объекта пунктов меню и проверке типов.
*/
//
// Наследника класса Menu!! (В моём случае наследника класса createMenu)
// Попробуем. Дабы ускориться, будем копировать и изменять.
function createSubMenu(myId, myClassName, myMenuItems) {
    // Подключаем главный класс Menu
    createMenu.call(this);
    this.id = myId;
    this.className = myClassName;
    this.menuSubItems = myMenuItems;
}
// Если честно, то не вижу смысла что-то менять в свойствах класса, кроме имени.
createSubMenu.prototype = Object.create(createMenu.prototype);
createSubMenu.prototype.constructor = createSubMenu;
// Собстаенно описываем сам метод
createSubMenu.prototype.render = function() {
    // Данная функция должна вернуть готовое меню.
    // В переменную result записываем открывающий тег ul с id и классом
    let result = '<ul class="' + this.id + '" id="' + this.className + '">';
    // Далее надо внутрь положить пункты меню <li></li>
    // Для этого надо циклом for перебрать массив, который будет передан в items
    for (let item in this.menuSubItems) {
        // 
        //if (this.menuSubItems[item] instanceof createSubMenuItems) {
            //
            result += this.menuSubItems[item].render();
            //result += this.item.render();
        //} else {
        //    console.log('error');
        //}
...
// Теперь сущность для создания пунктов меню
function createSubMenuItems(myHref, myName) {
...
// Копирую долбанную конструкцию для создания метода render для пунктов меню.
createSubMenuItems.prototype = Object.create(createMenuItems.prototype);
createSubMenuItems.prototype.constructor = createSubMenuItems;
// И сам метод:
createSubMenuItems.prototype.render = function() {
    // Функция будет возвращать элемент li со ссылкой внутри
    return '<li class="' + this.className + '"><a href="' + this.href + '">' + this.name + '</a></li>';
}
// Создаём пункты меню
// Такой объект, как пункт меню у меня создаётся с помощью класса createMenuItems
let mSubItem1 = new createMenuItems("/about/history", "История");
let mSubItem2 = new createMenuItems("/about/now", "Настоящее");
let mSubItem3 = new createMenuItems("/about/future", "Будущее");
let mSubItems = {0: mSubItem1, 1: mSubItem2, 2: mSubItem3};
// Мне нужно что-то, что примет в себя сформированный код меню.
// Создаём этот объект через createMenu, передавая в класс (в ФК) id, className и массив, содержащий пункты меню
let submenu = new createSubMenu("mySubMenuClass", "mySubMenuId", mSubItems);

let parentLi = document.getElementsByClassName('menuItem')[1];
console.log(parentLi);
let li = document.getElementsByClassName('menuItem')[1].innerHTML;
parentLi.innerHTML = li + submenu.render();

// Короче, у меня получилось как-то так!
